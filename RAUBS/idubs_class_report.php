...
$stud_section=$class_name[0]['section'];

function check_grade($marks_obtained)
{
    $grade ;
    if($marks_obtained <= 20)                                 {$grade = 'E-2';   return $grade; }
    elseif($marks_obtained >= 21 && $marks_obtained <= 32)    {$grade = 'E-1';   return $grade; }
    elseif($marks_obtained >= 33 && $marks_obtained <= 40)    {$grade = 'D';     return $grade; }
    elseif($marks_obtained >= 41 && $marks_obtained <= 50)    {$grade = 'C-2';   return $grade; }
    elseif($marks_obtained >= 51 && $marks_obtained <= 60)    {$grade = 'C-1';   return $grade; }
    elseif($marks_obtained >= 61 && $marks_obtained <= 70)    {$grade = 'B-2';   return $grade; }
    elseif($marks_obtained >= 71 && $marks_obtained <= 80)    {$grade = 'B-1';   return $grade; }
    elseif($marks_obtained >= 81 && $marks_obtained <= 90)    {$grade = 'A-2';   return $grade; }
...
// Displaying all headers/headings...
    ...
    // echo '<tr><td colspan ="3">Max marks / Out of</td></tr>';
   ...
   foreach($student_array_data as $k => $v)      
    {
        ...
        foreach($v['exam_info']['data'] as $key => $marks)    
        {
            ...
            {echo "<td style='background-color:lightgrey' class ='nowrap'>".$marks_obtained.' / ' .check_grade(($total_obtained_marks*100)/$onesubject_total_marks). '</td>';} // marks obtained in one subject including all exams
            else
            {
            echo '<td  class ="nowrap">'.$marks_obtained.' / ' .check_grade(($total_obtained_marks*100)/$onesubject_total_marks). '</td>';
            }
...
//======================================================================================================================
//-----------------------------------------Class Report of classes 9th n 10th-------------------------------------
//======================================================================================================================

else
{
    //====================================Class Report of classes 9th(D) n 10th==========================================
    if (trim(strtolower($stud_section)) == 'd') {
    ...
    foreach ($temp_data as $k5 => $v5) {
        ...
        }elseif(strpos(strtolower($v5['subject_name']),'s.k') !== FALSE || strpos(strtolower($v5['subject_name']),'marathi') !== FALSE) {
            $combined_sub_groups['marathi_s.k'][$v5['subject_id']] = $v5['subject_id'];
            $combined_subs[$v5['subject_id']] = 'marathi_s.k';
        }
    }
    ...
    foreach ($temp_data as $k5 => $v5) {            //loop to add marks of 2 subjects for eg: Maths 1 + Maths 2, Hist + Geog
        if(strpos($v5['subject_name'],'1',0) !== FALSE) {
        ...
        } elseif(trim(strtolower($v5['subject_name']))=="marathi") {
            foreach ($temp_data as $k6 => $v6) {
                if(strtolower($v6['subject_name']) == 's.k') {
                    $temp3 = $v6;
                    $temp3['subject_name'] = 'Marathi & Sanskrit';
                    $temp3['combined'] = '1';
                    $temp3['no_total'] = '1';//In mark-sheet dont show total
            ...
            if(strtolower(trim($v5['subject_name'])) == 'geography') {
                    $v5['sub_divided'] = '1';
                    $v5['no_total'] = '1';//In mark-sheet dont show total
            }
            //If subject is s.k then mark it is as divided subject
            if(strtolower(trim($v5['subject_name'])) == 's.k') {
                    $v5['sub_divided'] = '1';
                    $v5['no_total'] = '1';//In mark-sheet dont show total
            }            
            $temp2[] = $v5;
...
// echo print_array($subject_order);die();
...
//Displaying student names and marks
   ...
   foreach($student_array_data as $k => $v)       
        ...
        foreach ($subject_order as $k1 => $v1) {
            ...
            if (isset($sub_wise_total[$v1])) {
                ...
                if (isset($combined_subs[$v1])) {
                    ...
                    $combined_obt = $sub_wise_total[$subject_ids[0]]['sub_obt']+$sub_wise_total[$subject_ids[1]]['sub_obt'];
                    $combined_max = $sub_wise_total[$subject_ids[0]]['sub_max']+$sub_wise_total[$subject_ids[1]]['sub_max'];
                    
                    $subj_obt_tot = ($combined_max > 200 ) ? ($combined_obt)/3 : $combined_obt/2;
                    $grand_total += round($subj_obt_tot);
                    
                    $subj_max_tot = ($combined_max > 200 ) ? ($combined_max)/3 : $combined_max/2;
                    $max_marks += $subj_max_tot;

                    ($subj_obt_tot >= 35) ? $no_of_passing_subjects++ : $style = 'font-weight:bolder;background:#e5e5e5;'; 
    ...
    //====================================Class Report of classes 9th(A,B,C) n 10th==========================================
    ?>
    <section style="margin-top:40px;text-align:center;" onload="setTimeout(myFunction(), 3000)">
        <style>
            pre {
...
//Displaying student names and marks
   ...
   foreach($student_array_data as $k => $v)       
        ...
        foreach ($subject_order as $k1 => $v1) {
            ...
            if (isset($sub_wise_total[$v1])) {
                if (isset($combined_subs[$v1])) {
                    ...
                    $grand_total += round($subj_obt_tot);
                    $max_marks += ($sub_wise_total[$subject_ids[0]]['sub_max']+$sub_wise_total[$subject_ids[1]]['sub_max'])/2;
                    ($subj_obt_tot >= 35) ? $no_of_passing_subjects++ : $style = 'font-weight:bolder;background:#e5e5e5;'; 
    ...
    }//ENd ELSE 9th CLass Section (A,B,C)
}//End ELSE 9th CLass
?>
</table>
</div>
